# WhatsApp AI Agent с автономными действиями

Это **автономный AI-агент** для WhatsApp, который не просто отвечает на вопросы, а **самостоятельно выполняет действия**: ищет информацию, регистрирует студентов, уведомляет менеджеров.

Агент принимает сообщения через **GREEN-API**, обрабатывает их через **OpenAI GPT-4o с Function Calling** и автономно использует инструменты для выполнения задач.

## 🤖 Главная фича: AI Agent с Function Calling

**Автономный агент** который сам принимает решения:
- 🔍 **Поиск в базе знаний** (Google Docs) - автоматически ищет информацию о курсах, ценах, расписании
- 📝 **Регистрация студентов** (Notion) - сам записывает на курсы при запросе
- 📢 **Уведомление менеджера** (WhatsApp) - эскалирует сложные вопросы и жалобы
- 📅 **Работа с датами** - учитывает актуальное время

> **Подробнее:** см. [AI_AGENT_README.md](AI_AGENT_README.md)

## 🚀 Основные возможности

- 🎙️ **Голосовые сообщения**: Распознавание речи с помощью OpenAI Whisper API
- 💬 **Текстовые сообщения**: Обработка и интеллектуальные ответы
- 🧠 **AI Agent**: Автономные действия через Function Calling
- 🔗 **n8n интеграция**: Сложные workflows и оркестрация
- 📚 **База знаний**: Динамические промпты из Google Docs
- 💾 **Двухуровневая память**: Redis (краткосрочная) + PostgreSQL (долгосрочная)
- 🌍 **Мультиязычность**: Русский, казахский, английский
- ⚡ **Асинхронная обработка**: Быстрая обработка сообщений
- 📊 **API для аналитики**: Endpoints для истории чатов
